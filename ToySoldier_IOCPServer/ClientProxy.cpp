...
ClientProxy::ClientProxy(const TCPSocketPtr InSock, const SocketAddress& inSocketAddress, const string& inName, int inPlayerId) :
	mSock(InSock),
...
void	ClientProxy::RespawnCatIfNecessary()
{
	...
	{
		static_cast<Server*> (Engine::sInstance.get())->SpawnUnitForPlayer(mPlayerId);
		mTimeToRespawn = 0.f;
	}
}
